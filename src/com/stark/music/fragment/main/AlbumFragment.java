...
import com.stark.adapter.SearchListViewAlbumAdapter;
...
public class AlbumFragment extends Fragment implements LoaderCallbacks<Cursor> {
	...
	public void setUpView() {
		...
		abmAdapter = new AlbumListViewAdapter(getActivity(),
				R.layout.album_list_item_layout, null, new String[] {},
				new int[] {}, 0);
		elasticListView.setAdapter(abmAdapter);
		elasticListView.setOnScrollListener(mScrollListener);
		elasticListView
				.setOnItemClickListener(new AlbumListItemClickListener());
		sideBar.setListView(elasticListView);
		titleTV.setText(R.string.album);
	...
	public void initSideBar() {
		if (itemNum > 30) {
			isTurnOnNav = true;
			abmAdapter.setTurnOnNav(isTurnOnNav);
	...
	public Loader<Cursor> onCreateLoader(int id, Bundle args) {
		String[] projection = { MediaStore.Audio.Media._ID, BaseColumns._ID,
				AlbumColumns.ALBUM, AlbumColumns.ARTIST,
				AlbumColumns.ALBUM_ART, AlbumColumns.NUMBER_OF_SONGS };
		Uri uri = Audio.Albums.EXTERNAL_CONTENT_URI;
		String sortOrder = "album_key"/* Audio.Albums.ALBUM */;
	...
	public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
		...
		mSongNum = data.getColumnIndexOrThrow(AlbumColumns.NUMBER_OF_SONGS);
		SortCursor sc = new SortCursor(data, MediaStore.Audio.Media.ALBUM);
		abmAdapter.changeCursor(sc);
		mCursor = sc;
		String string = this.getString(R.string.num_of_album);
	...
	OnScrollListener mScrollListener = new OnScrollListener() {
		...
		public void onScrollStateChanged(AbsListView view, int scrollState) {
			...
			case OnScrollListener.SCROLL_STATE_FLING:
				abmAdapter.setFlagBusy(true);
				break;
			case OnScrollListener.SCROLL_STATE_IDLE:
				abmAdapter.setFlagBusy(false);
	...
	private class AlbumListItemClickListener implements OnItemClickListener {
		...
		private SearchListViewAlbumAdapter slvaAdapter;
		...
		private void setList(Window window) {
			...
			slvaAdapter = new SearchListViewAlbumAdapter(getActivity(),
					R.layout.songs_list_item_layout, null, new String[] {},
					new int[] {}, 0);
			searchELV.setAdapter(slvaAdapter);
		...
		private void changeContainCursor(String contain, String[] colums) {
			...
			while (cursor.moveToNext()) {
				...
				Matcher matcher = Pattern.compile(wordReg).matcher(album);

				if (matcher.find()) {
					containWords = matcher.group();
					slvaAdapter.setContainWords(containWords);
					String title = cursor
							.getString(cursor
			...
			slvaAdapter.changeCursor(matrixCursor);
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			if (position == 1) {
				...
				long songId = mCursor.getLong(mSongIdIndex);
				...
				AlbumUiListFragment abmUiFragment = AlbumUiListFragment
						.newInstance(bundle);
				...
				ft.replace(R.id.content, abmUiFragment).commit();
				;
			}
